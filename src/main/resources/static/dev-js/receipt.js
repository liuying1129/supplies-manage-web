...
btnSave.onclick = function() {
	...
	$.ajax({
		...
		//默认值适合大多数情况。如果你明确指定$.ajax()的 content-type,那么它必定会发送给服务器（即使没有数据要发送）
		//contentType : "application/x-www-form-urlencoded",//application/json
		url : 'saveReceipt?unid='+unid+'&sjunid='+sjunid+'&vendor='+encodeURIComponent(encodeURIComponent(vendor))+'&djh='+encodeURIComponent(encodeURIComponent(djh))+'&ph='+encodeURIComponent(encodeURIComponent(ph))+'&yxq='+yxq+'&sl='+sl+'&dw='+encodeURIComponent(encodeURIComponent(dw))+'&rkrq='+rkrq+'&memo='+encodeURIComponent(encodeURIComponent(memo)),
		//预期服务器返回的数据类型。如果不指定，jQuery将自动根据 HTTP包 MIME信息来智能判断
		dataType : 'json',
		success : function(data) {			
